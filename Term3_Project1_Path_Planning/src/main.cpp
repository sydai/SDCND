...
bool lane_change (const vector<vector<double>> &sensor_fusion, int prev_size, double car_s, double dist_car_front, int lane, int lr) { 
...
int main() {
  ...
  string line;
  while (getline(in_map_, line)) { // read map_file_ line by line for way points (x, y, s, dx, dy)
  ...
  double ref_vel = 0.0; // mph // starting velocity before h.onMessage() (cyclic)

  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy, &lane, &ref_vel](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) { // must capture by reference of lane and ref_vel
    // "42" at the start of the message means there's a websocket message event.
    // The 4 signifies a websocket message
    // The 2 signifies a websocket event
    //auto sdata = string(data).substr(0, length);
    //cout << sdata << endl;// start in lane 1 (2nd lane from left (center lane of the 3 lanes))
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
        	...
        	// Main car's localization Data
          	...
          	auto sensor_fusion = j[1]["sensor_fusion"]; // vector<vector<double>> : vector<{id,x,y,vx,vy,s,d}>
            ...
            // when about to hit, decrease to a speed equal to front car
            if (emergency_stop) {
              ref_vel = follow_speed; 
            }

            // when too close, decrease with an acc 7m/s^2; when enough front space, increase speed with same acc until reach speed limit
            if (too_close) {
              ref_vel -= .224; // in mph; .224/1.6093/0.02 = 7 m/s^2 < 10 m/s^2 acc limit
            }
            else if (ref_vel < 49.5) {
              ref_vel += .224; // gradually acc from cold start corresponding init ref_vel = 0.0;
            }

            // when change_left flag is true no matter change_right flag value, treat change_left high priority
            if (change_left) {
              lane-=1;
            }
            if (change_right) {
            ...
            // create a list of widely spaced (x, y) waypoints, evenly space at 30m
            // later will interpolate these waypoints with a spline and fill it with more points that can control speed

            vector<double> ptsx;
            vector<double> ptsy;

            // reference x, y, yaw states (either start point as where the car currently is OR at previous path end point)
            double ref_x = car_x;
            double ref_y = car_y;
            double ref_yaw = deg2rad(car_yaw);

            // if previous size is almost empty, use car current position as starting reference
            if (prev_size < 2) {
